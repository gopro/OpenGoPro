...
async def test_write_command_correct_uuid_cmd_id(mock_ble_communicator: GoProBase):
    response = await mock_ble_communicator.ble_command.set_shutter(shutter=constants.Toggle.ENABLE)
...
async def test_ble_setting(mock_ble_communicator: GoProBase):
    response = await mock_ble_communicator.ble_setting.led.set(constants.LED_SPECIAL.BLE_KEEP_ALIVE)
    assert response["uuid"] == GoProUUID.CQ_SETTINGS
    assert response["packet"] == bytearray([SettingId.LED, 1, constants.LED_SPECIAL.BLE_KEEP_ALIVE])
