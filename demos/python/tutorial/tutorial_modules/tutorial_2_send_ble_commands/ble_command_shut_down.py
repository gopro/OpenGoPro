# ble_command_sleep.py/Open GoPro, Version 2.0 (C) Copyright 2021 GoPro, Inc. (http://gopro.com/OpenGoPro).
...
async def main(identifier: Optional[str]) -> None:
    ...
    event.clear()
    await client.write_gatt_char(COMMAND_REQ_UUID, bytearray([0x01, 0x04]))
    await event.wait()  # Wait to receive the notification response
    await client.disconnect()


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Connect to a GoPro camera, then put the camera to sleep.")
    parser.add_argument(
        "-i",
        "--identifier",
        type=str,
