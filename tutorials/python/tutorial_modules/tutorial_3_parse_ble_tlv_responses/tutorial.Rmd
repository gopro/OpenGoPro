---
title: 'Python Tutorial 3: Parsing BLE TLV Responses'
date: '`r Sys.Date()`'
author: Tim Camise
output:
    rmdformats::downcute:
...
[Open GoPro Interface](https://github.com/gopro/OpenGoPro) to parse BLE
...
> Note! It is required that you have first completed the [connect](../tutorial_1_connect_ble/tutorial.html#requirements) and [sending commands](../tutorial_2_send_ble_commands/tutorial.html) tutorials before going through this tutorial.
...
We must first connect as was discussed in the [connect tutorial](../tutorial_1_connect_ble/tutorial.html). When enabling notifications,
one of the notification handlers described in the following sections will be used.

# Response Overview

In the preceding tutorials, we have been using a very dumb response handling procedure where the notification
...
```{r, out.height='50%', fig.cap = "Sequence for Response <= 20 Bytes"}
mermaid(diagram = '
sequenceDiagram
  participant PC
  participant GoPro
...
[Open GoPro Interface](https://github.com/gopro/OpenGoPro/tree/main/docs/ble#packet-headers), responses can be
...
[Set Shutter](../tutorial_2_send_ble_commands/tutorial.html#set-shutter). It returned a response of
...
[previous tutorial](../tutorial_2_send_ble_commands/tutorial.html).
...
From the [commands reference](https://github.com/gopro/OpenGoPro/tree/main/docs/ble#commands-quick-reference), we can see that these are:
...
First, we send the command to the Command Request [UUID](https://github.com/gopro/OpenGoPro/tree/main/docs/ble#services-and-characteristics):
...
from the [interface description](https://github.com/gopro/OpenGoPro/tree/main/docs/ble#complex-command-responses),
...
quiz(
    ...
    answers = list(
        list("a", "20 bytes"),
        list("b", "256 bytes"),
        list("c", "There is no maximum size.")
    ),
    correct_answer = "a",
    answer_info = "Responses can be composed of multiple packets where each packet is at
    maximum 20 bytes."
)
```
...
quiz(
    ...
    correct_answer = "c",
    answer_info = "There is no limit on the amount of packets that can comprise a response."
)
```

```{r}
quiz(
    question = "How many packets are command responses?",
    answers = list(
        list("a", "Always 1 packet."),
        list("b", "Always multiple packets."),
        list("c", "Always 1 packet except for complex responses.")
    ),
    correct_answer = "c",
    answer_info = "Command responses are almost always 1 packet (just returning the status).
...
```{r}
quiz(
    question = "How many packets are setting responses?",
    answers = list(
        list("a", "Always 1 packet."),
        list("b", "Always multiple packets."),
        list("c", "Always 1 packet except for complex responses.")
    ...
    correct_answer = "a",
    answer_info = "Settings Responses only ever contain the command status. Furthermore, there
...
No. Create new response, then get its length.
...
```{r, out.height='50%', fig.cap = "Response Accumulation Algorithm"}
mermaid(diagram = '
graph TD
...
```{r}
quiz(
    question = "How do we know when a response has been completely received?",
    answers = list(
        list("a", "The stop bit will be set in the header."),
        list("b", "The response has accumulated length bytes."),
        list("c", "By checking for the end of frame (EOF) sentinel character.")
    ),
    correct_answer = "b",
    answer_info = "The length of the entire response is parsed from the first packet. We
...
introduce such queries until [the next tutorial](../tutorial_4_ble_queries/tutorial.html) so for now, only the
parsing of the response is important.

> Note! While multi-packet responses are almost always Query Responses, they can also be from Command Complex responses. In a real-world implementation, it is therefore necessary to check the received UUID to see how to parse.
...
it needs to be handled to account for the possibility that it may always be more than 1 packet.
...
```{r, out.height='80%', fig.cap = "Response Accumulation Algorithm"}
mermaid(diagram = '
graph TD
...
quiz(
    ...
    answers = list(
        ...
        list("b", "Always multiple packets."),
        list("c", "Always 1 packet except for complex responses."),
        list("d", "Can be 1 or multiple packets.")
    ),
    correct_answer = "d",
    answer_info = "Query responses can be one packet (if for example querying a specific
...
quiz(
    ...
    correct_answer = "d",
    answer_info = "Trick Question! 🤭 Every response will have each of the three fields above. The type
    of ID will vary by response (Command ID for Command responses, Query ID for Query Responses,
    and Setting ID for Settings) but an ID will always exist."
)
```
...
You can now parse any response that is received from the GoPro, at least if it is received uninterrupted. There
...
To learn more about queries, go to the [next tutorial](../tutorial_4_ble_queries/tutorial.html).
