...
[Open GoPro Interface](https://github.com/gopro/OpenGoPro) to query the camera's setting and status information
via BLE.

"Queries" in this sense are specifically procedures that are initiated by writing to the Query UUID and received
responses via the Query Response UUID. This will be described in more detail below.

> Note! It is required that you have first completed the [connect](../tutorial_1_connect_ble/tutorial.html#requirements), [sending commands](../tutorial_2_send_ble_commands/tutorial.html), and [parsing responses](../tutorial_3_parse_ble_tlv_responses/tutorial.html) tutorials before going through this tutorial.
...
We must first connect as was discussed in the [connect tutorial](../tutorial_1_connect_ble/tutorial.html).

We will also be using the **Response** class that was defined in the
[parsing responses](../tutorial_3_parse_ble_tlv_responses/tutorial.html) tutorial to accumulate and parse
notification responses to the Query Response [characteristic](https://github.com/gopro/OpenGoPro/tree/main/docs/ble#services-and-characteristics).
...
following [commands](https://github.com/gopro/OpenGoPro/tree/main/docs/ble#query-commands).

| Query ID | Request              | Query           |
| -------- | -------------------- | --------------- |
| 0x12     | Get Setting value(s) | length:12:xx:xx |
| 0x13     | Get Status value(s)  | length:13:xx:xx |

where **xx** are setting / status ID(s) and **length** is the total length of the query (not including the length).
There will be specific examples below.

> Note! Since the they are two separate commands, combination of settings / statuses can not be polled simultaneously.
...
The amount of notification responses will vary depending on the amount of settings that have been queried.
...
responses as was described in [parsing query responses](../tutorial_3_parse_ble_tlv_responses/tutorial.html#query-responses)
...
> Note! When we are storing the updated setting, we are just taking the first byte (i..e index 0). A real-world implementation would need to know the length (and type) of the setting / status response by the ID. For example, sometimes settings / statuses are bytes, words, strings, etc.
...
[parsing query responses](../tutorial_3_parse_ble_tlv_responses/tutorial.html#query-responses) tutorial
...
quiz(
    ...
    answers = list(
        list("a", "Concatenate a Get Setting Value command and a Get Status command with the relevant ID's."),
        list("b", "Concatenate the Get All Setting and Get All Status commands."),
        list("c", "It is not possible.")
    ),
    correct_answer = "c",
    answer_info = "It is not possible to concatenate commands. This would result in an unknown sequence of bytes
...
The relevant [commands](https://github.com/gopro/OpenGoPro/tree/main/docs/ble#query-commands) are:

| Query ID | Request                           | Query           |
| -------- | --------------------------------- | --------------- |
| 0x52     | Register updates for setting(s)   | length:52:xx:xx |
| 0x53     | Register updates for status(es)   | length:53:xx:xx |
| 0x72     | Unregister updates for setting(s) | length:72:xx:xx |
| 0x73     | Unregister updates for status(es) | length:73:xx:xx |
...
> Note! The response to the Register command also contains the current setting / status value
...
The we register for updates when the resolution setting changes:
...
in [Multiple Simultaneous Query Polls].
...
quiz(
    ...
    correct_answer = "a",
    answer_info = "While there is probably not a good reason to do so, there is nothing preventing polling
    in this manner."
)
```
...
quiz(
    ...
    correct_answer = "b",
    answer_info = "It is possible for push notifications to contain multiple setting ID's if both setting ID's have
...
proceed to the [next tutorial](../tutorial_5_connect_wifi/tutorial.html).
