...
output:
    rmdformats::downcute:
        ...
        in_header: ../../../../tools/rdocs/header.txt
---
...
> Note! It is required that you have first completed the [Connecting to Wifi](../tutorial_5_connect_wifi/tutorial.html) and [Sending WiFi Commands](../tutorial_6_send_wifi_commands/tutorial.html) tutorials.
...
# Get Media List { .tabset .tabset-fade}

Now that we are are connected via WiFi, we will get the media list using the same procedure
to send HTTP commands as in the [previous tutorial](../tutorial_6_send_wifi_commands/tutorial.html).

## Send Request

We get the media list via the
[Get Media List command](https://github.com/gopro/OpenGoPro/tree/main/docs/wifi#commands-quick-reference).
This command will return a JSON list of all of the media files (pictures, videos) on the camera with corresponding information about each media file.
...
[Open GoPro Specification](https://github.com/gopro/OpenGoPro/tree/main/docs/wifi#media-list-format).
...
Where as all of the WiFi commands described until now have returned JSON responses, most of
...
[Download Media](https://github.com/gopro/OpenGoPro/tree/main/docs/wifi#downloading-media). Specifically, we
will be downloading a photo. The camera must have at least one photo in its media list in order for this to
work.

First, we get the media list as in [Get Media List]. Then we search through the list of file names in
...
> Note! The endpoint will use **videos** for both photos and videos
...
[Get Media GPMF](https://github.com/gopro/OpenGoPro/tree/main/docs/wifi#commands-quick-reference). More
...
The next command we will be sending is
[Get Media Screennail ](https://github.com/gopro/OpenGoPro/tree/main/docs/wifi#downloading-media).
...
The next command we will be sending is
[Get Media thumbnail ](https://github.com/gopro/OpenGoPro/tree/main/docs/wifi#downloading-media).
Specifically, we will be getting the thumbnail for a photo. The camera must have at least one photo in its
media list in order for this to work.

> Note! There is a separate command (shown above) to get a media **screennail**
...
See the previous tutorial's [troubleshooting section](../tutorial_6_send_wifi_commands/tutorial.html#troubleshooting).
...
inspiration check out the demos in the **demos** folder.
